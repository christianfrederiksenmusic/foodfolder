        ...
        <div className="mt-3 space-y-2">
          {confirmedItems.length ? (
            confirmedItems.map((name, idx) => (
              <div
                key={`${name}-${idx}`}
                className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-transparent px-3 py-2"
              >
            ...
            <div className="rounded-2xl border border-slate-200 bg-transparent px-4 py-4 text-sm text-slate-700">
...
const originalBytes = useMemo(
  ...
  async function addConfirmed() {
    const raw = normalizeItem(newConfirmedItem);
    if (!raw) return;

    const itemsDa = await ensureDaCanonical(raw);
    if (!itemsDa.length) return;

    setConfirmedItems((prev) => dedupeCaseInsensitive([...(prev || []), ...itemsDa]));
    setNewConfirmedItem("");
}
...
async function addShopping() {
  const raw = normalizeItem(newShoppingItem);
  if (!raw) return;
  ...
  async function commitMissingFromSelectedRecipe() {
    if (!recipesResult || (recipesResult as any).ok !== true) return;
    const recs = Array.isArray((recipesResult as any).recipes) ? (recipesResult as any).recipes : [];
    if (!recs.length) return;

    const r = recs[Math.max(0, Math.min(selectedRecipeIdx, recs.length - 1))];
    const missRaw = dedupeCaseInsensitive(getRecipeMissingItems(r));

    // Canonicalize missing-items to Danish (marketLang) so offers/matching keep working.
    const missDa = await ensureDaCanonicalListRef.current(missRaw);

    // Do NOT add items you already have (fridge + pantry)
    const pantryLocal = loadPantryLocal();
    const haveSet = new Set(
      dedupeCaseInsensitive([...(confirmedItems || []), ...(pantryLocal || [])])
        .map((x) => normalizeItem(x))
        .filter(Boolean)
    );

    const filtered = missDa
      .map((x) => normalizeItem(x))
      .filter((x) => x && !haveSet.has(x));
...
<h1 className="mt-2 text-3xl font-semibold leading-tight text-slate-900">
        ...
        <section className="space-y-6">
          <div className="rounded-3xl border border-amber-300/70 bg-amber-100/70 shadow-sm backdrop-blur">
            <div className="border-b border-amber-300/80 bg-amber-200/70 px-6 py-5">
              <div className="text-center">
                <div className="flex justify-center">
                  <span className="rounded-full border border-amber-400 bg-amber-300/80 px-2 py-0.5 text-[11px] font-semibold text-amber-900">
                    {lang === "da" ? "Trin 1" : "Step 1"}
                  </span>
                </div>
                ...
                <div className="mx-auto mt-1 max-w-3xl text-sm text-slate-600">
                  {lang === "da"
                    ? "Ét samlet flow: Spisekammer, scan og bekræftede ingredienser. Her styrer du, hvad du har hjemme."
                    : "One combined flow: Pantry, scan, and confirmed ingredients. This is where you manage what you have at home."}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4 lg:col-span-4">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-slate-900">
                        {lang === "da" ? "Scan køleskab/skab" : "Scan fridge/pantry"}
                      </div>
                      <div className="mt-1 text-xs text-slate-600">
                        {lang === "da"
                          ? "Upload et foto og scan ingredienser direkte ind i lagerflowet."
                          : "Upload a photo and scan ingredients directly into the inventory flow."}
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="h-10 flex-1 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-800 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {lang === "da" ? "Vælg foto" : "Choose photo"}
                    </button>
                    <button
                      type="button"
                      onClick={callFridge}
                      disabled={apiBusy || busy || recipesBusy || !chosen.dataUrl}
                      className="h-10 rounded-xl bg-blue-600 px-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {apiBusy ? (lang === "da" ? "Scanner..." : "Scanning...") : (lang === "da" ? "Scan" : "Scan")}
                    </button>
                  </div>
                  ...
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-2">
                    {chosen.dataUrl ? (
                      ...
                      <div className="flex h-40 items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white px-3 text-center text-xs text-slate-500">
                        {lang === "da" ? "Køleskab, fryser eller skab" : "Fridge, freezer, or pantry"}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-2 text-xs text-slate-500">
                    {lang === "da"
                      ? "Tip: Tag billedet oppefra i godt lys for bedre genkendelse."
                      : "Tip: Take the photo from above in good light for better detection."}
                  </div>
                  ...
                  {versionShort ? (
                    <div className="mt-3 text-xs text-slate-400">sha: {versionShort}</div>
                  ) : null}
                </div>
              </div>
              ...
              <div className="space-y-4 lg:col-span-8">
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-slate-900">
                        {lang === "da" ? "Bekræft ingredienser" : "Confirm ingredients"}
                      </div>
                      <div className="mt-1 text-xs text-slate-600">
                        {lang === "da"
                          ? "Ret listen efter scan. Det er denne liste, opskrifterne bygger på."
                          : "Edit the scanned list. This is the list recipes are built from."}
                      </div>
                    </div>
                  </div>
                  <div className="mt-3 rounded-2xl border border-slate-200 bg-white px-3 py-3">
                    <div className="rounded-2xl border border-amber-200/70 bg-amber-50/60 px-3 py-3">
                      {renderFridgeEditor()}
                    </div>
                  </div>
                  {false && (
                            ...
                            {sortedItems.map((it, idx) => (
                              ...
                              >
                                ...
                                {typeof it.confidence === "number" ? (
                                  <div className="text-xs font-semibold text-slate-500">{Math.round(it.confidence * 100)}%</div>
                                ) : null}
                              </div>
                            ))}
                          </div>
          ...
          <div className="rounded-3xl border border-amber-200/80 bg-amber-50/80 shadow-sm backdrop-blur">
            <div className="border-b border-amber-200/80 bg-amber-100/80 px-6 py-5">
              <div className="text-center">
                <div className="flex justify-center">
                  <span className="rounded-full border border-amber-300 bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-800">
                    {lang === "da" ? "Trin 2" : "Step 2"}
                  </span>
                </div>
            ...
            <div className="px-6 py-6">
              <div className="text-xs font-semibold uppercase tracking-wide text-slate-600">{recipeModeUi.label}</div>
              ...
              >
                {recipesBusy ? t(lang, "generating") : t(lang, "make_recipes")}
              </button>
          ...
          <div className="rounded-3xl border border-yellow-200/80 bg-yellow-50/85 shadow-sm backdrop-blur">
            <div className="border-b border-yellow-200/80 bg-yellow-100/80 px-6 py-5">
              <div className="text-center">
                <div className="flex justify-center">
                  <span className="rounded-full border border-yellow-300 bg-yellow-100 px-2 py-0.5 text-[11px] font-semibold text-yellow-800">
                    {lang === "da" ? "Trin 3" : "Step 3"}
                  </span>
                </div>
                ...
                <div className="mx-auto mt-1 max-w-3xl text-sm text-slate-600">
                  {lang === "da"
                    ? "Lav din indkøbsliste og find bagefter butikker og tilbud på det, du mangler."
                    : "Build your shopping list and then find stores and offers for what you need."}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 px-6 py-6 lg:grid-cols-12">
              <div className="space-y-4 lg:col-span-5">
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  ...
                  <div className="mt-1 text-xs text-slate-600">{t(lang, "add_from_recipe_desc")}</div>
