...
export default function Page() {
  ...
  const [constraints, setConstraints] = useState<string>("hurtigt, få ingredienser, ingen fancy udstyr");
  ...
  const chosenBytes = useMemo(() => dataUrlByteSize(chosen.dataUrl), [chosen.dataUrl]);
...
export default function Page() {
  ...
  async function callRecipes() {
    ...
    if (!apiResult || apiResult.ok !== true || apiResult.items.length === 0) {
      setRecipesResult({ ok: false, error: "Kør analyse først (vi mangler ingredienser)." });
      return;
    }
    ...
    <main className="min-h-screen bg-white text-slate-900">
      ...
      <div className="mx-auto max-w-6xl px-5 py-10">
        <header className="mb-8 flex items-start justify-between gap-6">
          <div>
            <div className="text-xs font-semibold tracking-[0.18em] text-slate-500">QARTIGO</div>
            <h1 className="mt-2 text-3xl font-semibold leading-tight text-slate-900">FoodFolder</h1>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-3xl border border-slate-200 bg-white/80 shadow-sm backdrop-blur">
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
              <div>
                <div className="text-base font-semibold text-slate-900">Billede</div>
                <div className="mt-1 text-sm text-slate-600">Vælg et foto og kør analyse.</div>
              </div>
            ...
            <div className="px-6 py-6">
              ...
              >
                ...
                <div className="relative">
                  {chosen.dataUrl ? (
                    <img src={chosen.dataUrl} alt="Preview" className="h-[340px] w-full bg-white object-contain" />
                  ) : (
                    <div className="flex h-[340px] flex-col items-center justify-center gap-3">
                      ...
                      <div className="text-xs text-slate-500">JPG/PNG/HEIC - vi optimerer automatisk payloaden</div>
                    </div>
                  )}
                </div>
              </button>
              ...
              <div className="mt-5 flex flex-col gap-2">
                <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  ...
                  <div className="text-sm font-medium text-slate-900">{pickedInfo ? pickedInfo.name : "Ingen"}</div>
                </div>

                <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3">
                  <div className="text-sm text-slate-600">Payload</div>
                  <div className="text-sm font-medium text-slate-900">{chosen.dataUrl ? fmtKB(chosenBytes) : "—"}</div>
                </div>
              </div>

              {error ? (
                <div className="mt-5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3">
                  <div className="text-sm font-semibold text-rose-900">Fejl</div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white/80 shadow-sm backdrop-blur">
            <div className="border-b border-slate-200 px-6 py-5">
              <div className="text-base font-semibold text-slate-900">Ingredienser</div>
              <div className="mt-1 text-sm text-slate-600">Efter analysen kan du generere opskrifter.</div>
            </div>

            <div className="px-6 py-6">
              {!apiResult ? (
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-10 text-center">
                  <div className="text-sm font-semibold text-slate-900">Ingen analyse endnu</div>
                  <div className="mt-2 text-sm text-slate-600">Upload et billede og tryk “Analyser”.</div>
                </div>
              ) : apiResult.ok === false ? (
                <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4">
                  <div className="text-sm font-semibold text-rose-900">{apiResult.error}</div>
                ...
                <>
                  ...
                  <div className="mt-6 rounded-2xl border border-slate-200 bg-white px-4 py-4">
                    <div className="text-sm font-semibold text-slate-900">Lav opskrifter</div>
                    <div className="mt-2 text-sm text-slate-600">Skriv constraints og generér forslag.</div>
                    ...
                    <div className="mt-6">
                      {recipesResult.ok === false ? (
                        <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4">
                          ...
                          <div className="mt-2 text-sm text-rose-800">
                            {recipesResult.requestId ? `requestId: ${recipesResult.requestId}` : ""}
                          </div>
                        </div>
                      ) : (
                          ...
                          {recipesResult.recipes.map((r, i) => (
                            <div key={i} className="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                              <div className="flex flex-wrap items-baseline justify-between gap-3">
                                ...
                                <div className="text-xs text-slate-500">
                                  {typeof r.time_minutes === "number" ? `${r.time_minutes} min` : ""}
                                  {typeof r.servings === "number" ? `  |  ${r.servings} pers` : ""}
                                </div>
                              </div>
                              {r.summary ? <div className="mt-2 text-sm text-slate-600">{r.summary}</div> : null}

                              <div className="mt-3 text-sm font-semibold text-slate-900">Ingredienser</div>
                              <ul className="mt-2 space-y-1 text-sm text-slate-700">
                                {r.ingredients.map((ing, k) => (
                                  <li key={k} className="flex justify-between gap-3">
                                    <span className="truncate">{ing.item}</span>
                                    <span className="shrink-0 text-slate-500">{ing.amount}</span>
                                  </li>
                                ))}
                              </ul>

                              <div className="mt-4 text-sm font-semibold text-slate-900">Trin</div>
                              ...
                              {Array.isArray(r.tags) && r.tags.length ? (
                                <div className="mt-3 flex flex-wrap gap-2">
                                  {r.tags.map((t, k) => (
                                    <span key={k} className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-700">
                                      {t}
                                    </span>
                                  ))}
                                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-10 text-center">
                  <div className="text-sm font-semibold text-slate-900">Ingen items</div>
                  <div className="mt-2 text-sm text-slate-600">Prøv et skarpere/beskåret billede.</div>
                </div>
              )}
            </div>
          </div>
        ...
        <footer className="mt-10 flex items-center justify-between text-xs text-slate-500">
          ...
          <div className="hidden sm:block">{versionShort ? `version: ${versionShort}` : ""}</div>
        </footer>
      </div>
    </main>
  );
