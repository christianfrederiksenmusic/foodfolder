      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm opacity-70">
            {queries.length ? fmt(t(props.lang, "offers_searching"), { q: queries.join(", ")  }) : t(props.lang, "offers_no_search_yet")}
          </p>
        </div>
        <div className="text-sm opacity-70">{loading ? t(props.lang, "offers_loading") : fmt(t(props.lang, "offers_count"), { n: String(offers.length)  })}</div>
      </div>

      {err && (
        <div className="mt-3 rounded-xl border border-red-300 bg-red-50 p-3 text-sm">{fmt(t(props.lang, "offers_error_prefix"), { err })}</div>
      )}

      {!err && !loading && offers.length === 0 && (
        <div className="mt-3 rounded-xl border border-black/10 bg-white p-3 text-sm opacity-80">
          {t(props.lang, "offers_none_found")}
        </div>
      )}
